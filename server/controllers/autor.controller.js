...
module.exports.get_all = (req, res) => {
    Autor.find().sort({nombre: 1}) //sort nos envia un diccionario o un objeto en la manera q queramos acomodar
        .then(autores => res.json(autores))
        .catch( err =>{
            console.log(err); //
            res.status(400).json(err); // me regresa un status del error para identificarlo
        });
}
